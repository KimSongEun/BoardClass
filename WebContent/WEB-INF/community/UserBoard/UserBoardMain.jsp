...
<div class="conwrap">
				...
				<tr>
					...
					<td class="obj">
						...
						<div class="util">
							...
							<div class="comt">댓글수: <span class="val">${board.commentCount }</span>
							</div>
						</div>
	...
	<div style="text-align: centerl; margin-top:20px;" class="pagination">
		<c:if test="${startPage > blockSize }">
			<a href="cu?pageNum=${startPage-blockSize}" style="color:#555;">[이전]</a>
		</c:if>
		<c:forEach var="i" begin="${startPage }" end="${endPage }">
			<c:if test="${pageNum == i }">
				<a href="cu?pageNum=${i }" style="color:#555;">[${i }]</a>
			</c:if>
		<c:if test="${pageNum != i }">
			<a href="cu?pageNum=${i }" style="color:#555;">[${i }]</a>
		</c:if>
		...
		<c:if test="${endPage < pageCnt }">
			<a href="cu?pageNum=${startPage + blockSize }" style="color:#555;">[다음]</a>
		</c:if>
 	</div>
